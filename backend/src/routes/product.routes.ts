...
const imageSchema = z
  ...
  .optional()
  .refine(
    (value) => !value || value.startsWith('data:') || /^https?:\/\//i.test(value),
    'La imagen debe ser una URL valida o un data URL',
  )
...
router.get('/', async (_req, res, next) => {
  try {
    const result = await pool.query(
      'SELECT id, nombre, descripcion, categoria, precio, imagen_url AS "imagenUrl", created_at AS "createdAt" FROM productos ORDER BY created_at DESC',
    )
