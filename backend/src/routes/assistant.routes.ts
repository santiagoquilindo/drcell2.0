  ...
  try {
    ...
    await pool.query(
      `
        ...
        VALUES ($1,$2,$3,$4)
      `,
      [req.body?.dispositivo ?? null, req.body?.motivo ?? null, req.body?.descripcion ?? null, error instanceof Error ? error.message : String(error)],
    ).catch(() => {})
    next(error)
  }
})

async function fetchSimilarCases(dispositivo?: string, motivo?: string) {
