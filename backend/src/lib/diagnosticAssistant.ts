...
type LocalRule = {
  ...
  tips_en_casa: string[]
  pasos_tecnico: string[]
  urgencia: DiagnosticResponse['urgencia']
...
function buildLocalSuggestion(input: DiagnosticInput) {
  ...
  const respuesta: DiagnosticResponse = matchedRule
    ? {
...
export async function generateDiagnosticSuggestion(input: DiagnosticInput) {
  if (!openai) {
  ...
  const similares = (input.similares ?? [])
    .map(
      (item, idx) =>
        `${idx + 1}. Tipo: ${item.tipo}. Dispositivo: ${item.dispositivo ?? 'N/D'}. Motivo: ${
          item.descripcion ?? 'N/D'
        }. Diagnóstico/resolución: ${item.diagnostico ?? item.resultado ?? 'N/D'}`,
    )
...
Agrega un tono empático y menciona que la revisión final se confirma en el laboratorio.`
  ...
  let parsed: DiagnosticResponse
  try {
    parsed = JSON.parse(content) as DiagnosticResponse
  } catch (error) {
    parsed = {
      resumen: content,
      probables_causas: [],
