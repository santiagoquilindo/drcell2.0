...
function drawHeader(doc, invoice) {
    ...
    if (invoice.clienteIdentificacion)
        doc.text(`Identificación: ${invoice.clienteIdentificacion}`);
    if (invoice.clienteTelefono)
        doc.text(`Teléfono: ${invoice.clienteTelefono}`);
    if (invoice.clienteEmail)
        doc.text(`Email: ${invoice.clienteEmail}`);
    if (invoice.clienteDireccion)
        doc.text(`Dirección: ${invoice.clienteDireccion}`);
    doc.moveDown();
}
...
const formatCOP = (value) => currencyFormatter.format(value);
