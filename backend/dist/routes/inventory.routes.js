...
router.patch('/:id', requireAdmin, async (req, res, next) => {
    try {
        ...
        if (data.nombre !== undefined)
            pushSet('nombre', data.nombre);
        if (data.categoria !== undefined)
            pushSet('categoria', data.categoria);
        if (data.proveedorId !== undefined)
            pushSet('proveedor_id', data.proveedorId ?? null);
        if (data.stockActual !== undefined)
            pushSet('stock_actual', data.stockActual);
        if (data.stockMinimo !== undefined)
            pushSet('stock_minimo', data.stockMinimo);
        if (data.precioCompra !== undefined)
            pushSet('precio_compra', data.precioCompra);
        if (data.precioVenta !== undefined)
            pushSet('precio_venta', data.precioVenta);
        if (data.descripcion !== undefined)
            pushSet('descripcion', data.descripcion ?? null);
        values.push(id);
        const result = await pool.query(`
