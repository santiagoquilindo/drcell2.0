...
router.post('/', async (req, res, next) => {
    ...
    try {
      ...
      `, [
        ...
        const devolucionId = inserted.rows[0].id;
...
router.patch('/:id', async (req, res, next) => {
    try {
        ...
        if (data.motivo !== undefined)
            pushField('motivo', data.motivo);
        if (data.diagnostico !== undefined)
            pushField('diagnostico', data.diagnostico);
        if (data.proveedorId !== undefined)
            pushField('proveedor_id', data.proveedorId ?? null);
        if (data.clienteId !== undefined)
            pushField('cliente_id', data.clienteId ?? null);
        if (data.slaProveedor !== undefined)
            pushField('sla_proveedor', new Date(data.slaProveedor));
        if (fields.length === 0) {
...
async function fetchReturn(id, client = pool) {
...
async function insertMovement(client, devolucionId, data) {
    await client.query(`
...
async function insertHistory(client, devolucionId, estado, comentario) {
    await client.query(`
      INSERT INTO devolucion_historial (devolucion_id, estado, comentario, actor)
      VALUES ($1,$2,$3,'sistema')
    `, [devolucionId, estado, comentario]);
}
async function nextReturnCode(client) {
    const result = await client.query(`SELECT LPAD(nextval('devolucion_codigo_seq')::text, 4, '0') AS seq`);
