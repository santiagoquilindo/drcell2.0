...
router.get('/', async (req, res, next) => {
    ...
    try {
        ...
        if (estado && statusEnum.options.includes(estado)) {
            values.push(estado);
...
async function createClientFromPayload(conn, payload) {
...
async function generateRepairCode(conn) {
    const result = await conn.query(`SELECT LPAD(nextval('repair_ticket_seq')::text, 4, '0') AS seq`);
    const now = new Date();
    const code = `RPR-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-${result.rows[0].seq}`;
    return code;
}
